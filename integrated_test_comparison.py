...
try:
    ...
    print("⚠️  Azure Blob Storage module not available. Install with: pip install azure-storage-blob")
...
def main():
    ...
    if BLOB_STORAGE_AVAILABLE:
        ...
        print(f"{'='*70}")
        # Upload with the filename expected by the API
        blob_url = upload_excel_to_blob(DEGRADED_OUTPUT_FILE, blob_name='degraded_responses.xlsx')
        if blob_url:
            print(f"  ℹ️  Blob Storage URL: {blob_url}")
            print(f"  ℹ️  API will serve this data at: GET /api/degraded-responses")
    else:
        print(f"\n  ℹ️  Azure Blob Storage integration disabled (module not imported)")
